...
def infix_to_postfix(infix):
    ...
    for step, char in enumerate(infix):
        ...
        result["steps"].append({
            "step": step + 1,
            "symbol": char,
            "stack": ''.join([item.data for item in stack_traverse(step_stack.top)]) if step_stack.top else 'Empty',
            "output": ''.join(output),
        })

        print(f"{step + 1}\t|\t{char}\t|\t{''.join([item.data for item in stack_traverse(step_stack.top)]) if step_stack.top else 'Empty'}\t|\t{''.join(output)}")
    ...
    while not stack.is_empty():
        ...
        print(f"{step_second_loop}\t|\t-\t|\t{''.join([item.data for item in stack_traverse(step_stack.top)]) if step_stack.top else 'Empty'}\t|\t{''.join(output)}")
